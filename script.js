...
function handleClearCompleted() {
    ...
    const confirmed = confirm(`Are you sure you want to delete ${completedCount} completed task(s)?`);
...
function renderTasks() {
    ...
    console.log(`📋 Rendered ${filteredTasks.length} tasks (filter: ${currentFilter})`);
}
...
function clearAllData() {
    const confirmed = confirm('⚠️ This will delete ALL your tasks permanently. Are you sure?');
...
function exportTasks() {
    ...
    link.download = `pink-todo-backup-${new Date().toISOString().split('T')[0]}.json`;
    ...
    reader.onload = function(e) {
        try {
            ...
            if (Array.isArray(importedTasks)) {
                const confirmed = confirm(`Import ${importedTasks.length} tasks? This will replace your current tasks.`);
